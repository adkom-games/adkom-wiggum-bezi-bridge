...
0c. For reference, the application source code is in "/Assets/_GAME/Scripts/". Create the directory if it doesn't exist. Do the same with other directories needed in _Game: Scripts, Materials, etc.
0d. Place a new documentation in pages located under a @Documentation folder.
0e. New source code should be placed in "/Assets/_Game/Source/".
...
2. After implementing functionality or resolving problems, run the tests (if present) for that unit of code that was improved. If functionality is missing then it's your job to add it as per the application specifications. 
